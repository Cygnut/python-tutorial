...
def init_logging(fmt=None):
    ...
    formatter = logging.Formatter(
        fmt or "%(asctime)s | %(name)25s | %(levelname)5s | %(message)s"
    )
...
class CheatsheetWriter:
    ...
    def __init__(self, current_dir=None):
        ...
        self._cheatsheet_path = os.path.join(
            self._current_dir, self._CHEATSHEET_FILENAME
        )
    ...
    def __write_cheatsheet(self, sections):
        ...
        for section in sections:
            with open(section.filepath) as f:
                ...
                content = spacer.join(
                    [f'""" Section {section.number} - {section.name} """', f.read()]
                )
